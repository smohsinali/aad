...
if __name__ == "__main__":
    ...
    for train, test in kf:
        ...
        means = np.zeros(max_trees)
        stdDevs = np.zeros(max_trees)
        ...
        for n_tree in range(1, max_trees+1):
            ...
            means[n_tree-1] = np.mean(scores[0:n_tree])
            stdDevs[n_tree-1] = np.std(scores[0:n_tree])
            print("n_estiamtor:%2d" % n_tree, "accuracy:%0.4f" % accuracy, "avg.:%0.4f" % means[n_tree-1], "stdDev:%0.4f" % stdDevs[n_tree-1])
        # print("scores:", scores)
